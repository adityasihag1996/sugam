...
class WebSearcher:
    ...
    def form_articles_mp(self, SEARCH_QUERIES, gs_api_key, gs_cse_id):
        ...
        def process_query(qry):
            results = self.google_search(qry, gs_api_key, gs_cse_id, num_results=num_results_for_search)
            urls = [result['link'] for result in results]

            with concurrent.futures.ThreadPoolExecutor() as executor:
                future_to_url = {executor.submit(self.get_clean_text_from_url, url): url for url in urls}
                local_article_idx = 0
